...
const listenerWarnings: { element: string; type: string; count: number }[] = [];
const thrashingWarnings: { description: string; time: number; count: number }[] = [];

// Event Listener Leak Detector state
const activeListeners = new Map<EventTarget, Map<string, Set<any>>>();
 ...
 */
function getElementDescription(target: EventTarget): string {
 ...
 */
function registerDOMRead(propName: string) {
...
function updateWorkerMemory() {
  ...
  if (typeof window !== 'undefined' && (window.performance as any).memory) {
    ...
    const gamesCount = (window as any).allGames ? (window as any).allGames.length : 120;
...
export function drawSearchLatencyHistogram() {
  ...
  if (statsDiv) {
    ...
    const avg = last10.reduce((a: number, b: number) => a + b, 0) / last10.length;
 ...
 */
export function addSearchQueryLatency(latencyMs: number) {
  const latencies = (window as any).searchLatencies || searchLatencies;
 ...
 */
export function initDiagnosticsDashboard() {
...
if (typeof window !== 'undefined') {
  (window as any).initDiagnosticsDashboard = initDiagnosticsDashboard;
  (window as any).addSearchQueryLatency = addSearchQueryLatency;
  (window as any).drawSearchLatencyHistogram = drawSearchLatencyHistogram;
  (window as any).drawLatencyHistogram = drawSearchLatencyHistogram;

  // Defaults
  (window as any).currentFps = 60.0;
  (window as any).frameCount = 0;
  (window as any).lastFpsUpdateTime = 0;
  (window as any).lastFrameTime = 0;
  (window as any).activeListenersCount = 0;
  (window as any).maxFrameDelta = 0;
  (window as any).searchLatencies = [];
  (window as any).panX = 0;
  (window as any).panY = 0;
  (window as any).zoomScale = 1.0;
  (window as any).topologyNodes = [];
  (window as any).topologyEdges = [];

  // Functions
  (window as any).getGameColor = function (game: any): string {
  ...
  (window as any).getNeighbors = function (game: any): any[] {
    if (!game || !game.governed_vectors) return [];
    const set = new Set(game.governed_vectors);
    const all = (window as any).allGames || [];
    return all.filter((g: any) => {
      if (g.game_id === game.game_id) return false;
      return g.governed_vectors && g.governed_vectors.some((v: string) => set.has(v));
    });
  };

  (window as any).buildTopologyGraph = function (focusGame: any) {
    if (!focusGame) return;
    const nodes: any[] = [];
    const edges: any[] = [];
    ...
    const focusNode = {
      ...
      vy: 0,
      radius: 15,
      color: (window as any).getGameColor(focusGame),
      game: focusGame,
      isDragging: false,
    };
    nodes.push(focusNode);

    const neighbors = (window as any).getNeighbors(focusGame);
    ...
    neighbors.forEach((neighbor: any, index: number) => {
      ...
      const node = {
        ...
        vy: 0,
        radius: 12,
        color: (window as any).getGameColor(neighbor),
        game: neighbor,
        isDragging: false,
      };
      nodes.push(node);
    ...
    for (let i = 0; i < neighbors.length; i++) {
      ...
      for (let j = i + 1; j < neighbors.length; j++) {
        ...
        const hasShare = nB.governed_vectors && nB.governed_vectors.some((v: string) => setA.has(v));
        if (hasShare) {
          edges.push({
            source: nodes[i + 1],
            target: nodes[j + 1],
    ...
    (window as any).topologyNodes = nodes;
    (window as any).topologyEdges = edges;
  };

  (window as any).updateTopologyPhysics = function () {
    const nodes = (window as any).topologyNodes || [];
    const edges = (window as any).topologyEdges || [];
  ...
  (window as any).runDiagnosticsLoop = function () {
    const now = (typeof performance !== 'undefined' ? performance : Date).now();
    const delta = now - ((window as any).lastFpsUpdateTime || 0);
    if (delta >= 500) {
      (window as any).currentFps = parseFloat(((((window as any).frameCount || 0) * 1000) / delta).toFixed(1));
      (window as any).frameCount = 0;
      (window as any).lastFpsUpdateTime = now;
    }
    const fpsEl = document.getElementById('diagnostics-fps');
    if (fpsEl) {
      fpsEl.textContent = ((window as any).currentFps !== undefined ? (window as any).currentFps : 60.0).toFixed(1);
    }
    if ((window as any).updateDiagnosticsAlerts) {
      (window as any).updateDiagnosticsAlerts();
    }
    if ((window as any).updateMemoryDisplay) {
      (window as any).updateMemoryDisplay();
    }
  };

  (window as any).updateDiagnosticsAlerts = function () {
    const alertsEl = document.getElementById('diagnostics-alerts') || document.getElementById('diag-active-alerts');
    if (!alertsEl) return;

    const alerts: string[] = [];
    const fps = (window as any).currentFps !== undefined ? (window as any).currentFps : 60;
    const listenersCount =
      (window as any).activeListenersCount !== undefined ? (window as any).activeListenersCount : 0;
