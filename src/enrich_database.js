...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'TTRPGRulesDatabaseEnricher/1.0 (contact: micha@example.com)' } }, (res) => {
      let data = '';
...
async function run() {
  ...
  for (let i = 0; i < gamesList.length; i += batchSize) {
    ...
    console.log(`Querying batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(gamesList.length / batchSize)} (indices ${i} to ${i + slice.length - 1})...`);
        ...
        for (const pageId in pages) {
          ...
          if (matchItem) {
            ...
            const isCurated = game.governed_vectors.some(vec => {
              ...
              return exp && !exp.includes("resolves") && !exp.includes("Initiative order") && !exp.includes("Features structured") && !exp.includes("governed by");
            });
