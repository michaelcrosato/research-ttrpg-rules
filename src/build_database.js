...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https
      .get(
        url,
        { headers: { 'User-Agent': 'TTRPGRulesDatabaseHarvester/1.0 (contact: micha@example.com)' } },
        (res) => {
          let data = '';
...
function classifyGame(title, medium, year) {
  ...
  if (medium === 'ttrpg') {
    ...
    const isFantasy =
      /dungeon|dragon|fantasy|sword|sorcery|magic|elf|dwarf|rune|quest|myth|hero|lord|chronicle|legend/i.test(
        titleLower
      );
    const isSciFi = /star|space|galaxy|alien|sci-fi|cyberpunk|laser|robot|mech|lancer|tech|retro|ship/i.test(
      titleLower
    );
    const isHorror = /cthulhu|horror|dead|zombie|vampire|werewolf|shadow|dark|ghost|terror|monster|masquerade/i.test(
      titleLower
    );
    ...
    if (isFantasy) {
      ...
      addVector(
        ...
        `Character creation in ${title} is class-based, defining starting abilities, health pools, and special skill tracks.`
      );
      addVector(
        'combat.initiative.dexterity_based',
        `Initiative order in ${title} is rolled at combat start, modified by Dexterity or agility modifiers.`
      );
      addVector(
        'combat.melee.tactical',
        `Engaging in melee combat in ${title} requires managing weapon reach, positioning, and close-quarters modifiers.`
      );
      addVector(
        'combat.ranged.tactical',
        `Ranged attacks in ${title} require line-of-sight checks, range-band penalties, and ammunition tracking.`
      );
      addVector(
        'combat.movement.grid_based',
        `Tactical movement during encounters in ${title} is tracked on a square or hex grid grid to govern positioning.`
      );
      addVector(
        'simulation.magic.spell_slots',
        `Spellcasting in ${title} utilizes finite spell slots or mana pools that recharge after a rest.`
      );
      addVector(
        'logistics.survival.rations',
        `Characters in ${title} must track rations and water, suffering penalties if they run out during travel.`
      );
      addVector(
        'stealth.action.hide',
        `Includes specific stealth actions to hide from hostiles in ${title}, matched against perception thresholds.`
      );
    } else if (isSciFi) {
      primary_genre = 'Sci-Fi';
      subgenres = ['Space Opera', 'Cybernetics', 'Futuristic'];
      addVector(
        'character.character_creation.tags_traits',
        `Characters in ${title} select specific traits, cyberware, or tech modules to define their gameplay style.`
      );
      addVector(
        'combat.initiative.perception_based',
        `Combat start sequence in ${title} uses sensory or perception checks to determine action order.`
      );
      addVector(
        'combat.ranged.tactical',
        `Ranged firefights in ${title} feature cover bonuses, distance scaling, and high-tech ammunition types.`
      );
      addVector(
        'combat.damage.armor_ablation',
        `Armor in ${title} reduces incoming damage or gets depleted over time as combat progresses.`
      );
      addVector(
        'logistics.survival.rations',
        `Logistical tracking in ${title} includes managing starship supplies, oxygen, or energy cells.`
      );
      addVector(
        'stealth.vision.light_levels',
        `Governs how stealth interacts with sensor sweeps, night vision, and lighting conditions in ${title}.`
      );
    } else if (isHorror) {
      ...
      addVector(
        ...
        `Tracks fear, stress, or sanity deterioration in ${title} when characters face unnatural terrors.`
      );
      addVector(
        'stealth.action.hide',
        `Stealth is a primary survival mechanic in ${title}, with rules for hiding from hostile entities.`
      );
      addVector(
        'stealth.detection.passive_perception',
        `NPCs and monsters in ${title} have a detection threshold representing passive vigilance.`
      );
      addVector(
        'character.character_creation.questionnaire',
        `Character background in ${title} is partially established via questions about past trauma and relationships.`
      );
    } else {
      ...
      addVector(
        ...
        `Features a playbook or template system in ${title} to streamline character creation and roleplay themes.`
      );
      addVector(
        'combat.initiative.dexterity_based',
        `Initiative order in ${title} is checked at combat start to sequence player actions.`
      );
      addVector(
        'combat.melee.tactical',
        `Resolves melee attacks in ${title} using character strength, weapon properties, and defense modifiers.`
      );
    }
  } else {
    // Board Game classification
    const isWargame = /war|battle|combat|panzer|tactical|squad|general|campaign|army|victory|napoleon|waterloo/i.test(
      titleLower
    );
    const isEconomic =
      /trade|economic|capital|business|monopoly|stock|market|rail|merchant|industry|brass|agricola/i.test(titleLower);
    ...
    if (isWargame) {
      ...
      addVector(
        'combat.movement.grid_based',
        `Forces move across a grid or hex-based map in ${title}, calculating tactical distances and line of sight.`
      );
      addVector(
        'combat.ranged.tactical',
        `Units in ${title} engage in ranged combat using weapon ranges, accuracy tables, and cover values.`
      );
      addVector(
        'combat.damage.critical_wounds',
        `Combat losses in ${title} are tracked through hit steps, retreats, or step reduction on unit counters.`
      );
      addVector(
        'logistics.travel.marching_order',
        `Requires maintaining supply lines and lines of communication across the battlefield in ${title}.`
      );
    } else if (isEconomic) {
      ...
      addVector(
        ...
        `Allows direct negotiation and bartering of resources or cards between players in ${title}.`
      );
      addVector(
        'economy.management.resource_allocation',
        `Players in ${title} manage an economy of coins, materials, and production engines to maximize points.`
      );
      addVector(
        'character.progression.victory_points',
        `The game is scored through victory points in ${title}, accumulated through economic building and card play.`
      );
    } else if (isCoop) {
      ...
      addVector(
        'logistics.survival.rations',
        `Players in ${title} must consume food, maintain gear, or combat disease to survive the campaign boards.`
      );
      addVector(
        'combat.movement.grid_based',
        `Features grid or board movements to coordinate player actions against the AI deck in ${title}.`
      );
      addVector(
        'character.progression.victory_points',
        `Tracks team success or points accumulated before the defeat condition is triggered in ${title}.`
      );
    } else if (isAbstract) {
      ...
      addVector(
        'combat.movement.grid_based',
        `Pieces move on a rigid grid board in ${title} with deterministic rules and no random output.`
      );
      addVector(
        'combat.melee.tactical',
        `Pieces capture others in ${title} by landing on their grid coordinates under specific movement constraints.`
      );
    } else {
      ...
      addVector(
        'economy.management.resource_allocation',
        `Requires balancing hand limits, resource spending, and card drafting in ${title}.`
      );
      addVector(
        'character.progression.victory_points',
        `Endgame winner in ${title} is determined by aggregating victory points from cards, board position, and resources.`
      );
    }
  }
...
async function run() {
  ...
  if (fs.existsSync(registryPath)) {
    try {
      ...
      console.log(
        `Loaded existing registry.json: ${existingRegistry.ttrpg.length} TTRPGs, ${existingRegistry.board_game.length} Board Games.`
      );
    } catch (e) {
  ...
  fs.writeFileSync(
    registryNamesPath,
    JSON.stringify(
      finalNamesList.sort((a, b) => b.year - a.year || a.title.localeCompare(b.title)),
      null,
      2
    ),
    'utf8'
  ...
  console.log(
    `Saved registry.json: ${outputRegistry.ttrpg.length} TTRPGs, ${outputRegistry.board_game.length} Board Games.`
  );
