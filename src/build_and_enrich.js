...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'TTRPGRulesDatabaseBuilderAndEnricher/3.0 (contact: micha@example.com)' } }, (res) => {
      let data = '';
...
function semanticClassify(title, medium, year, description, extract) {
  ...
  const diceMatch = combinedText.match(/\b(\d+)\s*(?:sided|d\d+)?\s*dice\b/i) || combinedText.match(/\b(?:roll|rolling)\s+(?:a|two|three|four)\s+dice\b/i);
  const cardsMatch = combinedText.match(/\bdeck of\s+(\d+)?\s*cards\b/i) || combinedText.match(/\b(\d+)\s*cards\b/i) || combinedText.match(/\bcard game\b/i);
  ...
  if (medium === 'ttrpg') {
    ...
    if (rollAndMove) {
      ...
      addVector('combat.movement.grid_based', `Features tactical movement on a hexagonal grid map in ${title} to govern ranges, line of sight, and unit positioning.`);
    } else if (primary_genre === 'Wargame' || subgenres.includes('Miniatures') || /grid|map|board|square/i.test(combinedText)) {
...
async function run() {
  ...
  const enrichList = [];
  harvestedTtrpgs.forEach(g => { if (!g.description || g.isNew) enrichList.push({ game: g, medium: 'ttrpg' }); });
  harvestedBoardGames.forEach(g => { if (!g.description || g.isNew) enrichList.push({ game: g, medium: 'board_game' }); });
