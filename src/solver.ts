...
import { SandboxConflictRule, SandboxDetectedConflict } from './types';

// Dependency rules for cycle detection
interface DependencyRule {
  vector: string;
  from: string; // The attribute that depends on another
  to: string; // The attribute it depends on
}
...
// Mutually exclusive states definition
interface MutuallyExclusiveRule {
  id: string;
  category: string;
  vectors: string[];
  description: string;
...
const EXCLUSIVE_RULES: MutuallyExclusiveRule[] = [
  {
 ...
 */
function matchVector(selected: string, pattern: string): boolean {
 ...
 */
function getMatchingSelected(selectedVectors: string[], pattern: string): string[] {
 ...
 */
function detectDependencyCycles(selectedVectors: string[]): SandboxDetectedConflict[] {
  const activeEdges: { from: string; to: string; vector: string }[] = [];
  ...
  const adj: Record<string, { to: string; vector: string }[]> = {};
  ...
  const path: { from: string; node: string; vector: string }[] = [];
 ...
 */
export function analyzeLogicalConflicts(selectedVectors: string[]): SandboxDetectedConflict[] {
  const conflicts: SandboxDetectedConflict[] = [];
  ...
  for (const rule of EXCLUSIVE_RULES) {
    ...
    const uniqueMatchingPatterns = rule.vectors.filter(
      (pattern) => getMatchingSelected(selectedVectors, pattern).length > 0
    );
  ...
  for (const [namespace, vectors] of Object.entries(namespaceGroups)) {
    if (vectors.length > 1) {
      ...
      const isAlreadyCritical = conflicts.some(
        (c) => c.rule.severity === 'critical' && vectors.every((v) => c.triggeringVectors.includes(v))
      );
...
if (typeof window !== 'undefined') {
  (window as any).analyzeLogicalConflicts = analyzeLogicalConflicts;
}
