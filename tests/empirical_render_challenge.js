...
async function runTests() {
  try {
    ...
    if (limitBypassViolation) {
      console.log(`⚠ VIOLATION: Synchronous rendering of 100 games took ${renderTime100.toFixed(2)}ms, exceeding the 8ms layout/render frame budget!`);
    } else {
    ...
    if (batchViolation) {
      console.log(`⚠ VIOLATION: At least one progressive rendering batch exceeded the 8ms frame budget! Max batch time: ${maxBatchTime.toFixed(2)}ms`);
    } else {
      console.log(`✔ PASS: All progressive rendering batches executed within ${maxBatchTime.toFixed(2)}ms (under 8ms limit).`);
    }
    ...
    const mockDictResults = Array.from(allMockVectors).map(vec => ({
      vector: vec,
      games: allGamesList.filter(g => g.governed_vectors && g.governed_vectors.includes(vec)).map(g => ({
        game_id: g.game_id,
        title: g.title,
        medium: g.medium,
        year: g.year
    ...
    console.log(`- Time to render all dictionary domains (${mockDictResults.length} vectors): ${dictDuration.toFixed(2)} ms`);
    ...
    if (dictDuration > 8.0) {
      console.log(`⚠ VIOLATION: Rendering the entire vector dictionary blocked the main UI thread for ${dictDuration.toFixed(2)}ms (budget: 8ms)!`);
    } else {
    ...
    if (autoDuration > 8.0) {
      console.log(`⚠ VIOLATION: Rendering autocomplete suggestions blocked the main UI thread for ${autoDuration.toFixed(2)}ms!`);
    } else {
    ...
    if (compareDuration > 8.0) {
      console.log(`⚠ VIOLATION: Rendering Venn comparison blocked the main UI thread for ${compareDuration.toFixed(2)}ms (budget: 8ms)!`);
    } else {
    ...
    if (postMessageCount > 2) {
      ...
      console.log(`✔ PASS: Debounce successfully throttled high-frequency typing to ${postMessageCount} execution(s).`);
    }
