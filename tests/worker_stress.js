...
async function startTestSuite() {
  try {
    ...
    for (const q of queries) {
      ...
      console.log(`  Avg: ${stats.avg.toFixed(3)}ms | Median: ${stats.median.toFixed(3)}ms | P95: ${stats.p95.toFixed(3)}ms | Min: ${stats.min.toFixed(3)}ms | Max: ${stats.max.toFixed(3)}ms`);
    }
    ...
    console.log(`  Avg: ${vecStats.avg.toFixed(3)}ms | Median: ${vecStats.median.toFixed(3)}ms | P95: ${vecStats.p95.toFixed(3)}ms`);
    ...
    console.log(`  Avg: ${domStats.avg.toFixed(3)}ms | Median: ${domStats.median.toFixed(3)}ms | P95: ${domStats.p95.toFixed(3)}ms`);
    ...
    console.log(`  Avg: ${allDomStats.avg.toFixed(3)}ms | Median: ${allDomStats.median.toFixed(3)}ms | P95: ${allDomStats.p95.toFixed(3)}ms`);
    ...
    const matchedRelevanceOrder = ['cyberpunk_red_2045_chronicle_book_2026', 'coriolis_empyrean_canticle_2e_edition_2026'];
    ...
    if (sortingDeviation) {
      console.log("⚠ SORTING DEVIATION CONFIRMED: Games autocomplete returned database insertion order instead of search relevance order.");
      console.log("  Reason: games.filter() filters the raw dataset array sequentially, ignoring the order in the matchedIds array.");
    } else {
