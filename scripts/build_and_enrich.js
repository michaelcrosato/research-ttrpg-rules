...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    https
      .get(
        url,
        { headers: { 'User-Agent': 'TTRPGRulesDatabaseBuilderAndEnricher/3.0 (contact: micha@example.com)' } },
        (res) => {
          let data = '';
...
function semanticClassify(title, medium, year, description, extract) {
  ...
  if (medium === 'ttrpg') {
    ...
    const isSciFi = /science fiction|sci-fi|space|galaxy|cyberpunk|mecha|lancer|futuristic|robot|steampunk/i.test(
      combinedText
    );
    ...
    if (isFantasy) {
      ...
      else if (/space|galaxy|opera/i.test(combinedText)) subgenres.push('Space Opera');
      else if (/steampunk/i.test(combinedText)) subgenres.push('Steampunk');
    ...
    const isEconomic =
      /economic|property|trade|trading|business|market|stock|commerce|industrial|merchant|building/i.test(combinedText);
  ...
  if (/tile placement|tile-placement|tile-laying/i.test(combinedText)) subgenres.push('Tile Placement');
  ...
  if (/roll-and-write|roll and write/i.test(combinedText)) subgenres.push('Roll-and-Write');
  if (/steampunk/i.test(combinedText)) subgenres.push('Steampunk');
  if (/apocalypse|post-apocalyptic/i.test(combinedText)) subgenres.push('Post-Apocalyptic');
  ...
  const playersMatch =
    combinedText.match(
      /\b(two|three|four|five|six|\d)\s*(?:to|-)\s*(?:two|three|four|five|six|eight|ten|\d+)?\s+players\b/i
    ) || combinedText.match(/\bfor\s+(\d+|two|three|four|five|six)\s+players\b/i);
  const diceMatch =
    combinedText.match(/\b(\d+)\s*(?:sided|d\d+)?\s*dice\b/i) ||
    combinedText.match(/\b(?:roll|rolling)\s+(?:a|two|three|four)\s+dice\b/i);
  const cardsMatch =
    combinedText.match(/\bdeck of\s+(\d+)?\s*cards\b/i) ||
  ...
  if (medium === 'ttrpg') {
    ...
    if (/perception/i.test(combinedText)) {
      addVector(
        ...
        `Initiative order in ${title} is determined by sensory or perception checks at combat start.`
      );
    } else if (/card/i.test(combinedText) && cardsMatch) {
      addVector(
        'combat.initiative.card_based',
        `Initiative order in ${title} is determined by drawing cards from a deck, sequencing player actions.`
      );
    } else {
      addVector(
        'combat.initiative.dexterity_based',
        `Initiative order in ${title} is rolled at combat start${diceInfo}, modified by Dexterity or agility modifiers.`
      );
    }
    ...
    if (hexMatch) {
      addVector(
        ...
        `Features tactical movement on a hexagonal grid map in ${title} to govern ranges, line of sight, and unit positioning.`
      );
    } else if (subgenres.includes('Dungeon Crawl') || /grid|map|square|board/i.test(combinedText)) {
      addVector(
        'combat.movement.grid_based',
        `Combat actions in ${title} assume tactical positioning mapped out on grids or square maps.`
      );
    }
    ...
    if (subgenres.includes('Rules-Light') || /playbook/i.test(combinedText)) {
      ...
      addVector(
        'character.character_creation.lifepath',
        `Features a lifepath character generation system in ${title} to roll or select chronological history events.`
      );
    } else {
      addVector(
        'character.character_creation.class_based',
        `Character creation in ${title} centers on choosing specific classes or archetypes that define starting skills and stats.`
      );
    }
    ...
    if (/experience|xp|level/i.test(combinedText)) {
      addVector(
        ...
        `Characters in ${title} earn experience points (XP) to advance levels, gain hit points, and unlock features.`
      );
    }
    ...
    if (/faction|reputation|alliance/i.test(combinedText)) {
      addVector(
        'politics.factions.reputation_points',
        `Features reputation or alliance tracks in ${title} to measure character standing with faction NPCs.`
      );
    }
    ...
    if (/weather|environment|hazard/i.test(combinedText)) {
      addVector(
        ...
        `Includes rules in ${title} for weather, temperature hazards, and hostile atmospheric conditions.`
      );
    }
  } else {
    // Board Games
    ...
    if (rollAndMove) {
      addVector(
        'logistics.travel.roll_and_move',
        `Features roll-and-move mechanics in ${title}${playersInfo} where movement distance is determined strictly by rolling dice.`
      );
    } else if (hexMatch) {
      addVector(
        'combat.movement.grid_based',
      ...
      addVector(
        'combat.movement.grid_based',
        `Units in ${title} navigate a tactical board or map grid to determine facing, line of sight, and ranges.`
      );
    }
    ...
    if (primary_genre === 'Wargame' || subgenres.includes('Miniatures')) {
      ...
      addVector(
        'combat.melee.tactical',
        `${title} resolves battle outcomes or actions using dice rolls${diceInfo} combined with card modifiers.`
      );
    }
    ...
    if (workerMatch) {
      addVector(
        'economy.management.worker_placement',
        `Features worker placement in ${title} where players allocate their workers to specific board action spaces.`
      );
    } else if (deckMatch) {
      addVector(
        'economy.management.deck_building',
        `Features deckbuilding in ${title} where players acquire cards to construct and optimize their personal drawing decks.`
      );
    } else if (draftMatch) {
      addVector(
        'economy.management.card_drafting',
        `Features card drafting in ${title}${playersInfo} where players select cards from hand passes to develop engines.`
      );
    } else {
      addVector(
        'economy.management.resource_allocation',
        `Players in ${title}${playersInfo} manage hand limits, token counts, and resource pools to build engines.`
      );
    }
    ...
    if (subgenres.includes('Trading') || /negotiat/i.test(combinedText)) {
      addVector(
        ...
        `Allows active negotiation, trade agreements, and direct resource bartering between players in ${title}${playersInfo}.`
      );
    }
    ...
    if (subgenres.includes('Deduction') || /bluff|deception/i.test(combinedText)) {
      addVector(
        'stealth.detection.passive_perception',
        `Secrets, hidden roles, or clues in ${title} must be uncovered using deduction loops and board triggers.`
      );
      addVector(
        'social.deception.insight',
        `Features bluffing and deception loops in ${title}${playersInfo} where players read others to spot hidden identifiers.`
      );
    }
    ...
    if (subgenres.includes('Cooperative')) {
      addVector(
        'logistics.survival.rations',
        `Cooperative play in ${title} requires players to manage health, sanity, or shared resources against board events.`
      );
    }
    ...
    addVector(
      ...
      `The winner of ${title}${playersInfo} is determined by scoring victory points from card combinations, tracks, and resources.`
    );
  }
...
async function run() {
  ...
  for (let idx = 0; idx < categories.length; idx++) {
    ...
    try {
      ...
      if (res.query && res.query.categorymembers) {
        ...
        if (added > 0) {
          console.log(
            `  [Wikipedia] Harvested ${added} new ${cat.medium === 'ttrpg' ? 'TTRPGs' : 'Board/Card games'} from ${cat.title}`
          );
        }
      }
    } catch (e) {
  ...
  harvestedTtrpgs.forEach((g) => {
    if (!g.description || g.isNew) enrichList.push({ game: g, medium: 'ttrpg' });
  });
  harvestedBoardGames.forEach((g) => {
    if (!g.description || g.isNew) enrichList.push({ game: g, medium: 'board_game' });
  });
    ...
    try {
      ...
      if (res.query && res.query.pages) {
        ...
        for (const pageId in pages) {
          ...
          if (redirectMap.has(targetTitle)) targetTitle = redirectMap.get(targetTitle);
          if (normalizedMap.has(targetTitle)) targetTitle = normalizedMap.get(targetTitle);
