...
let currentDictRenderJob = null;
// BGG mapping
const bggMechanicMapping = {
...
class TFIDFEngine {
    ...
    query(queryText, topK = 10) {
        ...
        const normalizedQueryVector = new Map();
...
class LocalSearchWorker {
    ...
    postMessage(data) {
        try {
            ...
            switch (type) {
                case 'init': {
                    const proceedWithData = (registryData) => {
                        const ttrpgs = (registryData.ttrpg || []).map((g) => ({
                            ...g,
                            medium: 'ttrpg',
                            governed_vectors_set: new Set(g.governed_vectors || []),
                        }));
                        const boardGames = (registryData.board_game || []).map((g) => ({
                            ...g,
                            medium: 'board_game',
                            governed_vectors_set: new Set(g.governed_vectors || []),
                        }));
                ...
                case 'search': {
                    ...
                    let results = this.games.filter((game) => {
                        const matchesSearch = !searchTerm ||
                            ...
                            (game.subgenres && game.subgenres.some((sub) => sub.toLowerCase().includes(searchTerm))) ||
                            (game.governed_vectors &&
                                game.governed_vectors.some((vec) => vec.toLowerCase().includes(searchTerm)));
                        const matchesMedium = medium === 'all' || game.medium === medium;
                        const matchesGenre = genre === 'all' || game.primary_genre === genre || (game.subgenres && game.subgenres.includes(genre));
                ...
                case 'compare': {
                    ...
                    const setA = new Set(gameA ? gameA.governed_vectors || [] : []);
                    const setB = new Set(gameB ? gameB.governed_vectors || [] : []);
    ...
    if (!game.governed_vectors || game.governed_vectors.length === 0) {
        ...
        <div class="modal-vectors-grid">
          ${grouped[domain]
                .sort()
                .map((vector) => {
                const explanation = (game.vector_explanations && game.vector_explanations[vector]) ||
...
function selectCompareGame(gameId, index, element) {
...
function highlightCompareColumn(colName) {
...
function setDictDomain(domain) {
    activeDictDomain = domain;
    renderDictSidebar();
    renderDictionary();
}
function toggleEditorVectorExplanation(vector, isChecked) {
...
function addCustomEditorVector() {
    ...
    const cb = document.getElementById(`check-vec-${val}`);
    if (cb) {
        cb.checked = true;
...
async function searchBGG() {
    const queryInput = document.getElementById('bgg-search-query');
    ...
    if (!queryInput || !resultsArea || !statusDiv)
        return;
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('BGG API response was not OK');
...
async function importBGGGame(bggId) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('BGG Thing API response was not OK');
        const xmlText = await response.text();
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(xmlText, 'text/xml');
        ...
        if (newGameTitleInput)
            newGameTitleInput.value = title || '';
        const newGameYearInput = document.getElementById('new-game-year');
        if (newGameYearInput)
            newGameYearInput.value = String(year);
        const newGameMediumSelect = document.getElementById('new-game-medium');
        if (newGameMediumSelect)
            newGameMediumSelect.value = 'board_game';
        // Assign primary genre and subgenres
        const newGameGenreInput = document.getElementById('new-game-genre');
        const newGameSubgenresInput = document.getElementById('new-game-subgenres');
        ...
        mechanics.forEach((mech) => {
            if (mech) {
                ...
                if (mappedVector) {
                    ...
                    // Find check item
                    const cb = document.getElementById(`check-vec-${mappedVector}`);
                    if (cb) {
                        cb.checked = true;
        ...
        checkedVectors.forEach((vec) => {
            const ta = document.querySelector(`#editor-explanations-inputs textarea[data-vector="${vec}"]`);
            if (ta) {
...
function printHighFidelity(ruleset) {
    ...
    if (ruleset.characterTemplate) {
        ...
        const statsRows = Object.entries(char.stats || {})
            .map(([k, v]) => `<td style="border: 1px solid #cbd5e1; padding: 0.4rem; text-align: center;"><strong>${k}</strong><br/>${v}</td>`)
            .join('');
        characterHtml = `
...
function switchImportPlatform(platform) {
    ...
    panes.forEach((p) => {
        ...
        if (pane)
            pane.style.display = p === platform ? 'block' : 'none';
        if (btn) {
            if (p === platform)
...
function mapAndCheckVectors(text) {
    ...
    mappings.forEach(({ words, vec }) => {
        if (words.some((w) => lowerText.includes(w))) {
            ...
            const cb = document.getElementById(`check-vec-${vec}`);
            if (cb) {
                cb.checked = true;
...
function parseItchHtmlAndPopulate(htmlText) {
    const parser = new DOMParser();
    ...
    if (titleEl)
        titleEl.value = title;
    const yearEl = document.getElementById('new-game-year');
    if (yearEl)
        yearEl.value = String(year);
    const mediumEl = document.getElementById('new-game-medium');
    if (mediumEl)
        mediumEl.value = medium;
    const genreEl = document.getElementById('new-game-genre');
    if (genreEl)
        genreEl.value = genre;
...
function parseItchHtmlFallback() {
    const ta = document.getElementById('itch-fallback-html');
    if (!ta)
        return;
    parseItchHtmlAndPopulate(ta.value);
}
async function importItchGame(itchId) {
    const statusDiv = document.getElementById('itch-search-status');
    if (!statusDiv)
        return;
    ...
    try {
        ...
        const subgenres = tags.filter((t) => t !== 'RPG' && t !== 'ttrpg').join(', ');
        const titleEl = document.getElementById('new-game-title');
        if (titleEl)
            titleEl.value = title;
        const yearEl = document.getElementById('new-game-year');
        ...
        if (genreEl)
            genreEl.value = genre;
        const subgenresEl = document.getElementById('new-game-subgenres');
        if (subgenresEl)
            subgenresEl.value = subgenres;
...
function parseDtrpgHtmlAndPopulate(htmlText) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(htmlText, 'text/html');
    const ogTitle = doc.querySelector('meta[property="og:title"]')?.getAttribute('content');
    let title = ogTitle || doc.querySelector('title')?.textContent || '';
...
function parseDtrpgHtmlFallback() {
    const ta = document.getElementById('drivethru-fallback-html');
    if (!ta)
        return;
    parseDtrpgHtmlAndPopulate(ta.value);
}
async function importDriveThruGame(dtrpgId) {
    const statusDiv = document.getElementById('drivethru-search-status');
    if (!statusDiv)
        return;
    ...
    try {
        ...
        const medium = categories.some((c) => c.toLowerCase().includes('board game')) ? 'board_game' : 'ttrpg';
...
async function searchWikipedia() {
    const queryInput = document.getElementById('wikipedia-search-query');
...
async function importWikipediaPage(pageId) {
    const statusDiv = document.getElementById('wikipedia-search-status');
    if (!statusDiv)
        return;
    statusDiv.textContent = `Fetching Wikipedia page details (ID: ${pageId})...`;
    try {
        const response = await fetch(`https://en.wikipedia.org/w/api.php?action=query&prop=extracts|categories&pageids=${pageId}&explaintext=1&format=json&origin=*`);
        if (!response.ok)
            throw new Error('API error');
        const data = await response.json();
        ...
        let year = 2026;
        const catString = categories.map((c) => c.title || '').join(' ');
        const yearMatch = catString.match(/introduced in (\d{4})/i) ||
    ...
    if (typeof Worker !== 'undefined') {
        try {
            ...
            worker.onerror = function (err) {
                ...
                globalThis.searchWorker = searchWorker;
...
function initEmbeddingsWorker() {
    ...
    if (typeof Worker !== 'undefined') {
        try {
            globalThis.embeddingsWorker = new Worker('dist/embeddings-worker.js');
        }
        catch (e) {
            globalThis.embeddingsWorker = new LocalEmbeddingsWorker();
        }
    }
    else {
        globalThis.embeddingsWorker = new LocalEmbeddingsWorker();
    }
    globalThis.embeddingsWorker.onmessage = function (e) {
        const data = e.data;
        if (data.type === 'ready') {
            if (bar)
                bar.style.width = '100%';
    ...
    globalThis.embeddingsWorker.postMessage({
        type: 'init',
        registryData: gamesData,
        modelName: 'MiniLM-L6-v2',
    });
}
function handleSemanticResults(matches) {
    const toggle = document.getElementById('semantic-search-toggle');
    const isSemantic = toggle ? toggle.checked : false;
    if (isSemantic) {
...
function checkAndApplyRRF() {
    if (!keywordResultsReceived || !semanticResultsReceived)
        return;
...
function handleWorkerAutocompleteResults(data) {
...
function handleWorkerCompareResults(data) {
    ...
    const onlyA = (data.onlyA || []);
    const onlyB = (data.onlyB || []);
  ...
  `;
    ...
    const container = resultsPanel.querySelector('.venn-diagram-container');
    if (!container)
        return;
    const hoverCard = container.querySelector('.venn-hover-card');
    if (!hoverCard)
        return;
    const segments = {
        ...
        el.addEventListener('mouseenter', () => {
            if (data.vectors.length === 0) {
                ...
                const list = data.vectors
                    ...
                    .map((v) => `• ${v}`)
                    .join('<br/>');
                const remaining = data.vectors.length > 3
    ...
    const listA = resultsPanel.querySelector('#compare-col-a .compare-list');
    if (listA) {
        if (onlyA.length === 0) {
    ...
    const listShared = resultsPanel.querySelector('#compare-col-both .compare-list');
    if (listShared) {
        if (shared.length === 0) {
    ...
    const listB = resultsPanel.querySelector('#compare-col-b .compare-list');
    if (listB) {
        if (onlyB.length === 0) {
        ...
        <div class="vector-game-list">
          ${matches
            ...
            if (fullGame && fullGame.vector_explanations) {
                const isParentNamespace = allGames.some((g) => g.vector_explanations &&
...
function handleWorkerAddGameDone(data) {
    const game = data.game;
    const medium = game.medium;
...
function saveGamesToCache(db, games) {
    return new Promise((resolve, reject) => {
        try {
...
async function loadDatabase() {
    try {
        ...
        if (cachedGames && cachedGames.length > 0) {
            ...
            const ttrpgs = (gamesData.ttrpg || []).map((g) => ({ ...g, medium: 'ttrpg' }));
            const boardGames = (gamesData.board_game || []).map((g) => ({ ...g, medium: 'board_game' }));
...
function processMetadata() {
    ...
    allGames.forEach((game) => {
        if (game.primary_genre)
            uniqueGenres.add(game.primary_genre);
        if (game.subgenres) {
...
function setupTabs() {
    ...
    let underline = container.querySelector('.tab-underline');
    if (!underline) {
    ...
    // Initial position
    const activeTab = (container.querySelector('.tab-btn.active') || tabs[0]);
    if (activeTab) {
...
function setupEventListeners() {
    ...
    if (btnExportFoundry) {
        btnExportFoundry.addEventListener('click', () => {
            if (activeModalGame)
                exportRulesetFoundry(activeModalGame);
        });
    }
    ...
    if (btnExportTTS) {
        btnExportTTS.addEventListener('click', () => {
            if (activeModalGame)
                exportRulesetTTS(activeModalGame);
        });
    }
    ...
    if (btnExportPDF) {
        btnExportPDF.addEventListener('click', () => {
            if (activeModalGame)
                exportActiveGamePDF(activeModalGame);
        });
    }
    ...
    if (btnSandboxExportPDF) {
        btnSandboxExportPDF.addEventListener('click', () => {
            if (sandboxSynthesizedRuleset)
                exportRulesetPDF(sandboxSynthesizedRuleset);
        });
    }
    ...
    if (btnExportMD) {
        btnExportMD.addEventListener('click', () => {
            if (activeModalGame)
                exportActiveGameMarkdown(activeModalGame);
        });
    }
    // Search bar typing
    ...
    if (minYear)
        minYear.addEventListener('change', handleYearChange);
    if (maxYear)
        maxYear.addEventListener('change', handleYearChange);
    // Sort select
...
function renderExplorer() {
    ...
    if (isSemantic) {
        ...
        if (globalThis.embeddingsWorker) {
            globalThis.embeddingsWorker.postMessage({
                type: 'query',
...
function createCardDOM(game) {
    ...
    const vectorsRemain = game.governed_vectors && game.governed_vectors.length > 3 ? ` (+${game.governed_vectors.length - 3} more)` : '';
    temp.innerHTML = `
...
function progressiveRenderDict(results, container) {
    ...
    let index = 0;
    function createDictCardDOM(item) {
...
function executeVectorSearch(vectorName) {
    const container = document.getElementById('vector-search-results');
    if (!container)
        return;
    if (!vectorName) {
...
function addNewGame() {
    ...
    const yearInput = document.getElementById('new-game-year');
    const mediumSelect = document.getElementById('new-game-medium');
    const genreInput = document.getElementById('new-game-genre');
    const subgenresInput = document.getElementById('new-game-subgenres');
    if (!titleInput || !yearInput || !mediumSelect || !genreInput || !subgenresInput)
        return;
...
function sandboxAnalyzeConflicts(selectedVectors) {
    const detected = [];
    for (const rule of SANDBOX_CONFLICT_RULES) {
        const matchedPatterns = rule.vectorPatterns.map((pattern) => selectedVectors.filter((v) => v === pattern || v.startsWith(pattern + '.')));
...
// --- Rules Synthesizer ---
const SANDBOX_DOMAIN_TEMPLATES = {
    combat: {
...
function sandboxVectorToLabel(vector) {
    return vector
        .split('.')
        .map((p) => p.replace(/_/g, ' '))
...
function sandboxSynthesizeRuleset(vectors, conflicts, vectorExplanations) {
    const domainGroups = {};
...
function sandboxGenerateCharacter(domainGroups) {
    const stats = {
        STR: 10,
        DEX: 10,
        CON: 10,
...
function sandboxRenderRulesetHTML(ruleset) {
...
function sandboxRollDice(notation) {
    ...
    if (typeof window !== 'undefined') {
        ...
        if (isWebGPUAvailable) {
            window.initWebGPUDicePhysics?.();
        }
        window.animateDiceRoll?.(sides, total + modifier);
    }
    return resultObj;
}
function sandboxFormatRollHTML(roll) {
...
function sandboxStatMod(value) {
    return Math.floor((value - 10) / 2);
}
function sandboxClassifyAction(input) {
    const lower = input.toLowerCase();
    if (/\b(attack|strike|hit|slash|stab|shoot|punch|kick|fight)\b/.test(lower))
        return 'attack';
    if (/\b(defend|block|dodge|parry|shield|protect|brace)\b/.test(lower))
        return 'defend';
    if (/\b(cast|spell|magic|invoke|channel|conjure|summon)\b/.test(lower))
        return 'cast';
    if (/\b(explore|search|look|examine|investigate|inspect|scout|check)\b/.test(lower))
        return 'explore';
    if (/\b(talk|persuade|negotiate|intimidate|charm|deceive|convince)\b/.test(lower))
        return 'social';
    if (/\b(climb|jump|swim|sneak|stealth|hide|pick\s*lock|craft|heal|medicine)\b/.test(lower))
        return 'skill';
    if (/\b(rest|sleep|camp|recover|meditate|eat|drink)\b/.test(lower))
        return 'rest';
    return 'unknown';
}
function sandboxGetRelevantStat(actionType, stats) {
    const mapping = {
        attack: 'STR',
        defend: 'DEX',
        cast: 'INT',
...
function sandboxCreateSession(ruleset) {
...
function sandboxAddMsg(session, role, content) {
    const msg = { role, content, timestamp: Date.now() };
    session.chatLog.push(msg);
    return msg;
}
function sandboxProcessAction(session, playerInput) {
    const newMsgs = [];
    session.turnNumber++;
...
function sandboxHandleAttack(session, mod, statName) {
    const msgs = [];
...
function sandboxHandleDefend(session, mod, statName) {
    const msgs = [];
    const roll = sandboxRollDice('1d20');
    const total = roll.finalResult + mod;
...
function sandboxHandleCast(session, mod, statName) {
    const msgs = [];
    ...
    if (total >= 12) {
        ...
        if (alive.length > 0) {
            ...
            if (target.hp <= 0) {
                target.hp = 0;
                target.alive = false;
                msgs.push(sandboxAddMsg(session, 'gm', `Arcane energy engulfs the ${target.name}! It dissolves into motes of light. (${damage} magical damage)`));
            }
            else {
                msgs.push(sandboxAddMsg(session, 'gm', `Your spell strikes the ${target.name}! (${damage} damage, ${target.hp}/${target.maxHp} HP remaining)`));
            }
        }
        else {
...
function sandboxHandleExplore(session, mod, statName) {
    const msgs = [];
    const roll = sandboxRollDice('1d20');
    const total = roll.finalResult + mod;
...
function sandboxHandleSocial(session, mod, statName) {
    const msgs = [];
    const roll = sandboxRollDice('1d20');
    const total = roll.finalResult + mod;
...
function sandboxHandleSkill(session, mod, statName, input) {
    const msgs = [];
    const lower = input.toLowerCase();
    let skillName = 'Skill Check';
    if (/heal|medicine/.test(lower))
        skillName = 'Medicine';
    else if (/sneak|stealth|hide/.test(lower))
        skillName = 'Stealth';
    else if (/climb/.test(lower))
        skillName = 'Athletics';
    else if (/pick\s*lock/.test(lower))
        skillName = 'Lockpicking';
    else if (/craft/.test(lower))
        skillName = 'Crafting';
    const roll = sandboxRollDice('1d20');
    const total = roll.finalResult + mod;
...
function sandboxHandleRest(session) {
    const msgs = [];
    if (session.encounterState.inCombat) {
...
function sandboxEnemyTurn(session) {
    const msgs = [];
    const alive = session.encounterState.enemies.filter((e) => e.alive);
    if (!session.encounterState.inCombat || alive.length === 0)
        return msgs;
    for (const enemy of alive) {
        ...
        if (defIdx >= 0)
            session.character.conditions.splice(defIdx, 1);
...
function sandboxRenderCharacterHTML(char) {
    let html = '';
    ...
    const hpColor = char.hitPoints <= char.maxHitPoints * 0.3
        ? '#ef4444'
...
function initializeSandbox() {
    const vectorInput = document.getElementById('sandbox-vector-input');
    ...
    const synthesizeBtn = document.getElementById('sandbox-synthesize-btn');
    const chatInput = document.getElementById('sandbox-chat-input');
    const chatSendBtn = document.getElementById('sandbox-chat-send');
    if (!vectorInput || !suggestionsContainer || !clearBtn || !synthesizeBtn || !chatInput || !chatSendBtn)
        return;
    // Vector autocomplete
    const debouncedVectorSearch = debounce((query) => {
        ...
        for (const vec of sandboxSelectedVectors) {
            ...
            for (const game of allGames) {
                if (game.vector_explanations && game.vector_explanations[vec]) {
                    ...
                    if (vectorExplanations[vec].length >= 5)
                        break;
                }
            }
        }
    ...
    const sendAction = () => {
        ...
        if (!input || !sandboxSession || sandboxSession.character.hitPoints <= 0)
            return;
    ...
    subTabButtons.forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (id === 'sub-tab-chat') {
                ...
                if (el)
                    el.style.display = 'flex';
                if (typeof window.calculateAndDrawSandboxProbability === 'function') {
                    window.calculateAndDrawSandboxProbability();
                }
            }
        });
    });
    ...
    if (btnHost)
        btnHost.addEventListener('click', () => window.hostLobby?.());
    if (btnJoin)
        btnJoin.addEventListener('click', () => window.joinLobby?.());
    if (btnSubmit)
        btnSubmit.addEventListener('click', () => window.submitLobbyToken?.());
    if (btnCopy)
        btnCopy.addEventListener('click', () => {
            const tokenOutput = document.getElementById('lobby-token-output');
            if (tokenOutput) {
                tokenOutput.select();
    ...
    const registerExportListeners = (prefix, rulesetGetter) => {
        ...
        if (fv)
            fv.addEventListener('click', () => window.exportRulesetFoundry?.(rulesetGetter()));
        if (r2)
            r2.addEventListener('click', () => window.exportRulesetRoll20?.());
        if (ts)
            ts.addEventListener('click', () => window.exportRulesetTTS?.(rulesetGetter()));
        if (pd) {
                        ...
                        const formattedRuleset = {
                            ...
                            sections: [
                                { heading: 'Description', rules: [game.description || ''] },
                                {
                                    ...
                                    rules: (game.governed_vectors || []).map((vector) => {
                                        const explanation = (game.vector_explanations && game.vector_explanations[vector]) ||
                                            'No detailed rule explanation recorded.';
        ...
        if (md)
            md.addEventListener('click', () => window.exportRulesetMarkdown?.(rulesetGetter()));
    };
    registerExportListeners('btn-sandbox-', () => window.sandboxSynthesizedRuleset);
...
function sandboxRenderSelectedVectors() {
    ...
    container.innerHTML = sandboxSelectedVectors
        .map((v) => `<span class="sandbox-vector-pill">${v}<span class="remove-vector" data-vector="${v}">&times;</span></span>`)
        .join('');
...
function sandboxRunConflictAnalysis() {
...
function sandboxUpdateSynthesizeBtn() {
    const btn = document.getElementById('sandbox-synthesize-btn');
    if (btn) {
        btn.disabled = sandboxSelectedVectors.length === 0;
    }
}
function sandboxRenderChatLog() {
    const chatLog = document.getElementById('sandbox-chat-log');
    if (!chatLog || !sandboxSession)
        return;
...
function sandboxAppendChatMessages(messages) {
    const chatLog = document.getElementById('sandbox-chat-log');
    if (!chatLog)
        return;
...
function sandboxRenderCharacterSheet() {
    const container = document.getElementById('sandbox-character-sheet');
    if (!container || !sandboxSession)
        return;
...
// ==========================================
async function initWebGPUDicePhysics() {
    ...
    try {
      ...
      `,
        ...
        const storageUsage = window.GPUBufferUsage?.STORAGE || 1;
        const copySrcUsage = window.GPUBufferUsage?.COPY_SRC || 2;
        const copyDstUsage = window.GPUBufferUsage?.COPY_DST || 4;
        const physicsBuffer = device.createBuffer({
            size: 64 * 4 * 16,
            usage: (storageUsage | copySrcUsage | copyDstUsage),
        });
...
function animateDiceRoll(sides, result) {
    if (typeof document === 'undefined')
        return;
    const canvas = document.getElementById('dice-physics-canvas');
    if (!canvas)
        return;
    canvas.style.display = 'block';
    const diceLogPanel = document.getElementById('dice-log-panel');
    if (diceLogPanel)
        diceLogPanel.style.display = 'block';
...
let lobbyRole = null;
function validateStateChangeProposal(proposal) {
    if (!proposal)
        return false;
    if (proposal.roll) {
    ...
    if (proposal.statChanges) {
        if (typeof proposal.statChanges.hitPoints === 'number') {
            ...
            const maxHp = window.sandboxSession?.character?.maxHitPoints || 100;
            if (hp < 0 || hp > maxHp) {
                return false;
            }
        }
...
async function proposeStateChange(action, roll, statChanges) {
...
function applyStateChange(proposal) {
    const session = window.sandboxSession;
    if (!session)
        return;
    if (proposal.statChanges) {
...
async function hostLobby() {
    ...
    if (statusText)
        statusText.textContent = 'Status: Hosting, gathering candidates...';
    if (statusDot)
        statusDot.style.background = '#f59e0b';
    if (tokenArea)
        tokenArea.style.display = 'flex';
    if (tokenLabel)
        tokenLabel.textContent = 'Your Host Token (send to player):';
    if (inputSection)
        inputSection.style.display = 'flex';
    if (inputPrompt)
        inputPrompt.textContent = 'Paste Player Join Token here:';
    ...
    peerConnection.onicecandidate = (event) => {
        if (!event.candidate) {
            ...
            if (statusText)
                statusText.textContent = 'Status: Host token ready. Send to Player.';
        }
    };
...
async function joinLobby() {
    ...
    const inputPrompt = document.getElementById('lobby-input-prompt');
    if (statusText)
        statusText.textContent = 'Status: Paste Host Token below:';
    if (statusDot)
        statusDot.style.background = '#f59e0b';
    if (tokenArea)
        tokenArea.style.display = 'flex';
    if (tokenLabel)
        tokenLabel.textContent = 'Your Join Token (send back to Host):';
    if (inputSection)
        inputSection.style.display = 'flex';
    if (inputPrompt)
        inputPrompt.textContent = 'Paste Host Token here:';
}
async function submitLobbyToken() {
    const inputVal = document.getElementById('lobby-token-input').value.trim();
    if (!inputVal)
        return;
    const statusText = document.getElementById('lobby-status-text');
    const statusDot = document.getElementById('lobby-status-dot');
        ...
        if (lobbyRole === 'host') {
            ...
            peerConnection.onicecandidate = (event) => {
                if (!event.candidate) {
                    ...
                    if (statusText)
                        statusText.textContent = 'Status: Join token ready. Send back to Host.';
                }
            };
            await peerConnection.setRemoteDescription(new RTCSessionDescription(sdpData));
...
function setupDataChannelHandlers(channel) {
    channel.onopen = () => {
        ...
        if (statusText)
            statusText.textContent = 'Status: Connected (Lobby Active)';
        if (statusDot)
            statusDot.style.background = '#10b981';
        if (roster)
            roster.style.display = 'flex';
        if (rosterList)
            rosterList.textContent = lobbyRole === 'host' ? 'Host (You), Player' : 'Host, Player (You)';
        const session = window.sandboxSession;
        if (session) {
...
// Exporters
function exportRulesetFoundry(ruleset) {
    if (!ruleset)
        return;
    const data = {
        name: ruleset.title,
    ...
    triggerDownload(JSON.stringify(data, null, 2), `${ruleset.title.replace(/[^a-z0-9]/gi, '_')}_foundry.json`, 'application/json');
}
function exportRulesetRoll20() {
    const char = window.sandboxSession?.character || {
        name: 'Unnamed Adventurer',
        level: 1,
        hitPoints: 10,
        maxHitPoints: 10,
    ...
    for (const [key, val] of Object.entries(char.stats)) {
        attributes.push({ name: key.toLowerCase(), current: val });
    }
    const data = {
        name: char.name,
        attributes,
    };
    triggerDownload(JSON.stringify(data, null, 2), `${char.name.replace(/[^a-z0-9]/gi, '_')}_roll20.json`, 'application/json');
}
function exportRulesetTTS(ruleset) {
    if (!ruleset)
        return;
    const char = window.sandboxSession?.character || ruleset.characterTemplate;
    const notebookData = [
        {
            title: 'Ruleset Documentation',
            body: ruleset.sections.map((s) => `## ${s.heading}\n${s.rules.join('\n')}`).join('\n\n'),
        },
        {
    ...
    triggerDownload(JSON.stringify(data, null, 2), `${ruleset.title.replace(/[^a-z0-9]/gi, '_')}_tts.json`, 'application/json');
}
function exportRulesetMarkdown(ruleset) {
    if (!ruleset)
        return;
...
function calculateExactStandard(count, sides, modifier) {
    let dp = { 0: 1 };
...
function calculateProbability(type, params) {
...
function drawProbabilityChart(canvas, dist, targetDC, hoverVal) {
    const ctx = canvas.getContext('2d');
    if (!ctx)
        return { points: [] };
    ...
    const getX = (val) => paddingLeft + ((val - minVal) / valRange) * plotWidth;
    const getYPdf = (p) => paddingTop + plotHeight - (p / maxPdf) * plotHeight;
    const getYCdf = (p) => paddingTop + plotHeight - p * plotHeight;
...
function getDedicatedProbabilityData() {
    const rollTypeEl = document.getElementById('prob-roll-type');
    if (!rollTypeEl)
        return null;
    const rollType = rollTypeEl.value;
    const explode = document.getElementById('prob-explode')?.checked || false;
    const modifier = parseInt(document.getElementById('prob-modifier')?.value || '0', 10);
    if (rollType === 'standard' || rollType === 'success') {
        const count = parseInt(document.getElementById('prob-dice-count')?.value || '1', 10);
        const sides = parseInt(document.getElementById('prob-dice-sides')?.value || '6', 10);
        const threshold = parseInt(document.getElementById('prob-threshold')?.value || '5', 10);
        ...
        const stepDice = {};
        const d4 = parseInt(document.getElementById('prob-step-d4')?.value || '0', 10);
        const d6 = parseInt(document.getElementById('prob-step-d6')?.value || '0', 10);
        const d8 = parseInt(document.getElementById('prob-step-d8')?.value || '0', 10);
        const d10 = parseInt(document.getElementById('prob-step-d10')?.value || '0', 10);
        const d12 = parseInt(document.getElementById('prob-step-d12')?.value || '0', 10);
        const d20 = parseInt(document.getElementById('prob-step-d20')?.value || '0', 10);
        if (d4 > 0)
            stepDice[4] = d4;
        if (d6 > 0)
            stepDice[6] = d6;
        ...
        if (d20 > 0)
            stepDice[20] = d20;
        const combineMethod = document.getElementById('prob-step-combine')?.value || 'sum';
...
function initializeProbabilityPanel() {
    const rollType = document.getElementById('prob-roll-type');
    ...
    if (!rollType || !standardInputs || !successInputs || !stepInputs || !explodeWrapper)
        return;
...
function getSandboxProbabilityData() {
    const formulaEl = document.getElementById('prob-dice-formula');
    const calcTypeEl = document.getElementById('prob-calc-type');
    if (!formulaEl || !calcTypeEl)
        return null;
    const formula = formulaEl.value;
    const type = calcTypeEl.value;
    const { count, sides, modifier } = parseDiceFormula(formula);
    const keepCount = parseInt(document.getElementById('prob-keep-count')?.value || '1', 10);
    const successThreshold = parseInt(document.getElementById('prob-success-threshold')?.value || '5', 10);
    return calculateProbability(type, {
        count,
        sides,
        modifier,
...
function calculateAndDrawSandboxProbability() {
    ...
    if (stddevEl)
        stddevEl.textContent = dist.stdDev.toFixed(2);
    const targetDCEl = document.getElementById('prob-target-dc');
    ...
    const canvas = document.getElementById('prob-chart-canvas');
    if (canvas) {
        drawProbabilityChart(canvas, dist, targetDC);
    }
}
function initializeSandboxProbability() {
    const calcType = document.getElementById('prob-calc-type');
    ...
    if (calcType) {
        calcType.addEventListener('change', () => {
            ...
            if (keepWrapper)
                keepWrapper.style.display = type === 'rollkeep' ? 'flex' : 'none';
            if (successWrapper)
                successWrapper.style.display = type === 'successpool' ? 'flex' : 'none';
        });
    }
    ...
    if (canvas) {
        ...
        const handleMouseMove = (e) => {
            ...
            if (closest) {
                ...
                if (distData) {
                    const targetDC = parseInt(document.getElementById('prob-target-dc')?.value || '10', 10);
...
function triggerProgrammaticPrint(html) {
...
function exportActiveGamePDF(game) {
    if (!game)
        return;
    const html = `
    <!DOCTYPE html>
...
function exportSandboxRulesetPDF(ruleset) {
    if (!ruleset)
        return;
    const html = `
    <!DOCTYPE html>
...
function exportRulesetPDF(ruleset) {
    exportSandboxRulesetPDF(ruleset);
}
function exportActiveGameMarkdown(game) {
    if (!game)
        return;
    let md = `# ${game.title}\n\n`;
...
class OmniGitVCS {
    ...
    mergeBranch(source, target) {
        ...
        sourceCommits.forEach((sc) => {
            sc.changes.forEach((sChg) => {
                targetCommits.forEach((tc) => {
                    tc.changes.forEach((tChg) => {
    ...
    resolveConflict(gameId, field, resolution) {
...
function resolveConflictSingle(gameId, field, resolution) {
...
function autoSaveHomebrewDraft() {
    const title = document.getElementById('homebrew-title')?.value || '';
    const vectors = document.getElementById('homebrew-vectors')?.value || '';
    const description = document.getElementById('homebrew-description')?.value || '';
    const publish = document.getElementById('homebrew-publish-toggle')?.checked || false;
...
function clearHomebrewForm() {
    const titleEl = document.getElementById('homebrew-title');
    const vectorsEl = document.getElementById('homebrew-vectors');
    const descEl = document.getElementById('homebrew-description');
    const publishEl = document.getElementById('homebrew-publish-toggle');
    if (titleEl)
        titleEl.value = '';
    if (vectorsEl)
        vectorsEl.value = '';
        ...
        request.onsuccess = () => {
            ...
            getRequest.onsuccess = () => {
                ...
                if (draft) {
                    ...
                    if (vectorsEl)
                        vectorsEl.value = draft.vectors || '';
                    if (descEl)
                        descEl.value = draft.description || '';
                    if (publishEl)
                        publishEl.checked = !!draft.publish;
                }
                resolve();
            };
...
async function publishHomebrew() {
    const title = document.getElementById('homebrew-title')?.value || 'My Homebrew Subsystem';
    const vectorsStr = document.getElementById('homebrew-vectors')?.value || '';
    const description = document.getElementById('homebrew-description')?.value || '';
...
function runSandboxSimulation() {
    const roundCountEl = document.getElementById('sim-round-count');
    const rounds = roundCountEl ? parseInt(roundCountEl.value) || 100 : 100;
    const charStats = window.sandboxSession?.character?.stats ||
        sandboxSession?.character?.stats || { STR: 10, DEX: 10, CON: 10, INT: 10, WIS: 10, CHA: 10 };
...
if (typeof window !== 'undefined') {
    window.initWebGPUDicePhysics = initWebGPUDicePhysics;
    window.animateDiceRoll = animateDiceRoll;
    window.validateStateChangeProposal = validateStateChangeProposal;
    window.proposeStateChange = proposeStateChange;
    window.gridToIso = gridToIso;
    window.isoToGrid = isoToGrid;
    window.isLineOfSightBlocked = isLineOfSightBlocked;
    window.initBattleMap = initBattleMap;
    window.exportRulesetFoundry = exportRulesetFoundry;
    window.exportRulesetRoll20 = exportRulesetRoll20;
    window.exportRulesetTTS = exportRulesetTTS;
    window.exportRulesetMarkdown = exportRulesetMarkdown;
    window.hostLobby = hostLobby;
    window.joinLobby = joinLobby;
    window.submitLobbyToken = submitLobbyToken;
    // Sandbox functions
    window.sandboxAnalyzeConflicts = sandboxAnalyzeConflicts;
    window.sandboxGenerateCharacter = sandboxGenerateCharacter;
    window.sandboxRollDice = sandboxRollDice;
    ...
    window.triggerProgrammaticPrint = triggerProgrammaticPrint;
    window.exportActiveGamePDF = exportActiveGamePDF;
    window.exportSandboxRulesetPDF = exportSandboxRulesetPDF;
    window.exportRulesetPDF = exportRulesetPDF;
    window.exportActiveGameMarkdown = exportActiveGameMarkdown;
    window.calculateProbability = calculateProbability;
    window.calculateAndDrawSandboxProbability = calculateAndDrawSandboxProbability;
    window.initializeSandboxProbability = initializeSandboxProbability;
    window.initializeProbabilityPanel = initializeProbabilityPanel;
    // VCS, Homebrew, and Sim bindings
    window.omniGitVCS = omniGitVCSInstance;
    window.resolveConflictSingle = resolveConflictSingle;
    window.completeConflictResolution = completeConflictResolution;
    window.autoSaveHomebrewDraft = autoSaveHomebrewDraft;
    window.clearHomebrewForm = clearHomebrewForm;
    window.loadHomebrewDraft = loadHomebrewDraft;
    window.publishHomebrew = publishHomebrew;
    window.runSandboxSimulation = runSandboxSimulation;
    window.sandboxRenderRulesetHTML = sandboxRenderRulesetHTML;
    window.LocalSearchWorker = LocalSearchWorker;
    window.LocalEmbeddingsWorker = LocalEmbeddingsWorker;
}
