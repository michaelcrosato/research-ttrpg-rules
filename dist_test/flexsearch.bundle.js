...
(function _f(self) {
        ...
        if (b || b === '') {
            if (typeof b === 'object' && b.constructor !== RegExp) {
                ...
                try {
                    ...
                    console.error('Your split configuration:', b, 'is not supported on this platform. It falls back to using simple whitespace splitter instead: /s+/.'),
                        (this.split = /\s+/);
                }
            }
            else {
        ...
        LatinSoundex: {
            ...
            finalize: function (a) {
                for (let b = 0; b < a.length; b++) {
                    ...
                    for (let f = 1, g; f < c.length &&
                        ((g = c.charAt(f)),
                            g === 'h' || g === 'w' || !(g = va[g]) || g === d || ((e += g), (d = g), e.length !== 4)); f++) { }
                    a[b] = e;
                }
            },
        },
    ...
    function Ma(a, c, b) {
        return c
            ...
            : a
                ...
                : new window.Worker(typeof b === 'string'
                    ? b
            ...
            if (n && J) {
                ...
                if (this.db && e) {
                    ...
                    for (let L = 0, O, P; L < n.length; L += 2) {
                        O = this.tag.get(n[L]);
                        if (!O) {
                            if ((console.warn("Tag '" + n[L] + ':' + n[L + 1] + "' will be skipped because there is no field '" + n[L] + "'."),
                                h)) {
                                continue;
                            }
                            else {
        ...
        if (v) {
            if (this.db && n && n.length && !F) {
                for (q = 0; q < n.length; q += 2) {
                    ...
                    if (!f) {
                        if ((console.warn("Tag '" + n[q] + ':' + n[q + 1] + "' was not found because there is no field '" + n[q] + "'."),
                            h)) {
                            continue;
                        }
                        else {
    ...
    w.import = function (a, c) {
        if (c) {
            switch ((typeof c === 'string' && (c = JSON.parse(c)),
                (a = a.split('.')),
    ...
    Fa(T.prototype);
    const Bb = typeof window !== 'undefined' &&
        (window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB), Cb = ['map', 'ctx', 'tag', 'reg', 'cfg'], Db = I();
    function Eb(a, c = {}) {
