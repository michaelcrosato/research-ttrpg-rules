    ...
    if (type === 'init') {
        try {
            ...
            if (!registryData) {
                try {
                    ...
                    throw new Error(`Failed to load registry: ${fetchErr instanceof Error ? fetchErr.message : String(fetchErr)}`);
                }
            }
            if (registryData) {
                const gamesList = [];
                if (registryData.ttrpg)
                    gamesList.push(...registryData.ttrpg);
                if (registryData.board_game)
                    gamesList.push(...registryData.board_game);
                try {
                    if (typeof transformers !== 'undefined') {
                        ...
                        for (const game of gamesList) {
                            ...
                            const embedding = l2Normalize(Array.from(output.data));
        ...
        if (useTransformers && extractor) {
            try {
                ...
                const queryEmbedding = l2Normalize(Array.from(output.data));
